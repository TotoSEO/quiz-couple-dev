import type { BlogArticleData } from '@/types/blog';
import { AUTHORS } from '@/data/blog/authors';

const article: BlogArticleData = {
  slug: 'amore-o-dipendenza-affettiva',
  title: "Lo amo, o ho paura di perderlo?",
  metaTitle: "Amore o dipendenza affettiva: le 7 differenze",
  metaDescription: "È facile confondere amare forte e avere paura di perdere. Le 7 differenze tra amore e dipendenza affettiva, e il test mentale che li distingue.",
  featuredImage: '/blog/amour-ou-dependance-affective.webp',
  featuredImageAlt: "Due cuori intrecciati, uno libero e l'altro circondato da una corda",
  publishedAt: '2026-10-06T09:45:00+02:00',
  author: AUTHORS['thomas'],
  excerpt: "L'amore e la dipendenza affettiva si somigliano enormemente visti dall'interno. Stessa intensità, stessi pensieri permanenti... eppure tutto li oppone.",
  introduction: `<p>È una domanda che non si osa troppo farsi, perché fa un po' paura: lo amo lui... o amo soprattutto il fatto che ci sia? È amore, questa cosa che mi stringe lo stomaco quando non risponde? O è altro?</p>
<p>La confusione è normale: <strong>visti dall'interno, l'amore intenso e la dipendenza affettiva si somigliano molto.</strong> In entrambi i casi pensi a lui di continuo, detesti la sua assenza, senti forte. La differenza si gioca altrove, sulla direzione: l'amore ti porta verso di lui, la dipendenza ti fa fuggire un vuoto.</p>
<p>Rendiamolo molto concreto, con le 7 differenze che distinguono i due e un esperimento mentale che vale più di qualsiasi discorso.</p>`,
  quickSummary: [
    "L'intensità non distingue niente: amore e dipendenza possono essere ugualmente forti.",
    "La vera differenza è la direzione: l'amore va verso l'altro, la dipendenza fugge un vuoto.",
    "Il marcatore centrale: cosa senti quando va tutto bene. La dipendenza ha paura anche nei giorni buoni.",
    "Il test mentale del weekend senza notizie distingue i due in cinque minuti.",
    "Amore e dipendenza convivono spesso: si può amare davvero E amare male.",
  ],
  sections: [
    {
      id: 'le-7-differenze',
      title: "Le 7 differenze che non ingannano",
      content: `<p>Prendi ogni riga e chiediti onestamente da quale lato ti riconosci più spesso. Non una volta ogni tanto: la maggior parte del tempo.</p>
<div><table><thead><tr><th></th><th>Amore</th><th>Dipendenza affettiva</th></tr></thead><tbody>
<tr><td>Quando va tutto bene</td><td>Te lo godi, semplicemente</td><td>Aspetti già il momento in cui si guasterà</td></tr>
<tr><td>La sua assenza</td><td>Ti manca, e la tua vita continua</td><td>Sei in astinenza, e la tua vita si ferma</td></tr>
<tr><td>Cosa ti spinge verso di lui</td><td>La voglia: la sua presenza aggiunge</td><td>La paura: la sua assenza minaccia</td></tr>
<tr><td>La tua identità</td><td>La relazione si aggiunge a chi sei</td><td>La relazione sostituisce chi sei</td></tr>
<tr><td>I disaccordi</td><td>Scomodi, ma possibili</td><td>Impensabili: cedi ancora prima di parlare</td></tr>
<tr><td>La sua libertà</td><td>La sua vita non ti minaccia</td><td>Ogni uscita senza di te è un allarme</td></tr>
<tr><td>La questione della fine</td><td>«Non voglio che ci lasciamo»</td><td>«Non posso permettere che mi lasci»</td></tr>
</tbody></table></div>
<p>L'ultima riga è la più importante. «Non voglio» è una scelta che parla di amore. «Non posso» è un'impossibilità che parla di sopravvivenza, e l'amore non dovrebbe essere una questione di sopravvivenza.</p>`,
    },
    {
      id: 'il-test-del-weekend',
      title: "L'esperimento mentale che distingue tutto: il weekend senza notizie",
      content: `<p>Chiudi gli occhi due minuti e immagina seriamente questo: parte per un intero weekend, con gli amici, in un posto senza campo. Nessuna notizia dal venerdì sera alla domenica sera. Niente di inquietante, niente di nascosto, solo silenzio.</p>
<p>Ora osserva cosa ti scatena. Non quello che vorresti provare: quello che provi davvero, adesso, solo a immaginarlo.</p>
<p><strong>Scenario A:</strong> non ti entusiasma, ti mancherà, ma vedi già cosa faresti del tuo weekend. Una serata con tua sorella, dormire fino a tardi, quella cosa che rimandi da settimane. La domenica sera saresti contenta di ritrovarlo e di raccontarvi tutto.</p>
<p><strong>Scenario B:</strong> solo a pensarci ti si stringe lo stomaco. Due giorni senza sapere? Sai già che controlleresti il telefono ogni dieci minuti, che il tuo weekend sarebbe una sala d'attesa, e che domenica ce l'avresti con lui, mentre ce l'hai con te per avercela con lui.</p>
<p>Hai appena misurato una cosa precisa: la parte del tuo attaccamento che è sicurezza e quella che è paura. Lo scenario B, a quel livello, non è più amore che trabocca, è un sistema d'allarme che ha preso i comandi. I <a href="/it/blog/sintomi-dipendenza-affettiva/">sintomi della dipendenza affettiva</a> dettagliano quel sistema pezzo per pezzo, se vuoi verificare più finemente.</p>`,
    },
    {
      id: 'i-due-convivono',
      title: "Il caso più frequente: entrambi insieme",
      content: `<p>Ora la sfumatura che conta di più: nella vita vera non è quasi mai l'uno o l'altro. <strong>Si può amare sinceramente qualcuno E amarlo in modo dipendente.</strong> L'amore è reale, il partner è scelto bene, è il modo di amare a essere intrappolato.</p>
<p>Come capirlo? Guarda cosa amplifica la tua relazione. Un amore sicuro amplifica il tuo meglio: osi di più, ridi più facilmente. Un attaccamento dipendente amplifica le tue paure: sorvegli tutto, ti scusi di continuo, e a poco a poco ti rimpicciolisci. Stessa persona, forse stesso amore, ma due direzioni opposte.</p>
<p>Ed è un'ottima notizia, in fondo. Perché se il problema non è la persona ma il modo di attaccamento, allora non c'è niente da rompere: c'è qualcosa da riparare, in te, per te. E <a href="/it/blog/superare-la-dipendenza-affettiva/">superare la dipendenza affettiva</a> è un percorso segnalato, esercizio dopo esercizio, che funziona benissimo senza lasciare nessuno.</p>
<aside class="blog-tip-box">
<p class="blog-tip-box-title">📌 E se fosse solo abitudine?</p>
<p>Esiste un terzo candidato che si dimentica: l'abitudine. Né la paura del vuoto, né lo slancio verso l'altro, solo il comfort di un quotidiano che gira. Se la tua domanda somiglia più a «lo amo ancora?» che a «perché ho così paura di perderlo?», è meno una storia di dipendenza che una questione di <a href="/it/test-amore-o-abitudine/">amore o abitudine</a>, e quella ha una risposta tutta sua.</p>
</aside>`,
    },
    {
      id: 'rispondere-alla-domanda',
      title: "Allora, amore o dipendenza? Rispondere una volta per tutte",
      content: `<p>Tre cose da fare per smettere di girare in tondo.</p>
<p>Riprendi le 7 differenze e conta le tue righe. Fai il test del weekend, davvero se se ne presenta l'occasione: le risposte immaginate sono utili, quelle vissute sono definitive. E parlane con qualcuno che ti conosceva prima di lui. Fagli LA domanda scomoda: «come mi trovi, da quando sto con lui?»</p>
<p>E se vuoi una misura tranquilla, strutturata, che non dipenda dal tuo umore della sera...</p>
<div class="blog-cta">
<p class="blog-cta-titre">Amore, abitudine o dipendenza: misuralo davvero</p>
<p class="blog-cta-texte">Una serie di domande valuta il tuo modo di attaccamento sulle situazioni concrete di questo articolo. Cinque minuti, un risultato chiaro, e finalmente una risposta che non cambia di giorno in giorno.</p>
<a class="blog-cta-btn" href="/it/test-dipendenza-affettiva/">Ottenere la mia risposta</a>
<p class="blog-cta-note">Gratis &middot; Senza registrazione &middot; Risultato immediato</p>
</div>
<p>Un'ultima cosa. Se scopri che il tuo modo di amare pende verso la dipendenza, non trasformarlo in un verdetto sulla tua storia. Non vuol dire né che il tuo amore sia falso, né che la tua coppia sia condannata. Vuol dire che ami con una vecchia paura appesa alla schiena, e che quella paura si può posare. Molte coppie attraversano questo lavoro e ne escono con un legame più leggero di prima. Anche la tua può.</p>
<a href="/it/blog/dipendenza-affettiva-nella-coppia/" class="blog-read-also"><span class="blog-read-also-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg></span><span class="blog-read-also-content"><span class="blog-read-also-label">Leggi anche</span><span class="blog-read-also-title">Dipendenza affettiva: quando amare diventa un bisogno vitale</span></span><svg class="blog-read-also-arrow" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14"/><path d="M12 5l7 7-7 7"/></svg></a>`,
    },
  ],
};

export default article;
