import type { BlogArticleData } from '@/types/blog';
import { AUTHORS } from '@/data/blog/authors';

const article: BlogArticleData = {
  slug: 'questions-a-poser-a-son-crush',
  title: "100 questions à poser à son crush",
  metaTitle: "100 questions à poser à son crush | Quiz Couple",
  metaDescription: "Marre des conversations qui retombent ? 100 questions à poser à ton crush, classées par situation : par message, pour rire, pour flirter, pour savoir si ça matche.",
  featuredImage: '/blog/questions-a-poser-a-son-crush.webp',
  featuredImageAlt: "Deux téléphones face à face reliés par des bulles de conversation et un cœur",
  publishedAt: '2026-09-01T11:34:00+02:00',
  author: AUTHORS['thomas'],
  excerpt: "Le plus dur avec un crush, c'est de trouver la question qui relance la conversation au bon moment. En voici 100, classées par situation.",
  introduction: `<p>Tu connais ce moment. La conversation tourne, les « ça va et toi » s'enchaînent, et tu sens que si personne ne relance avec autre chose, ça va gentiment mourir en « bonne soirée alors ». Avec un crush, chaque échange compte double ! Du coup, on se retrouve tous, un jour ou l'autre, à fixer son téléphone sans savoir quoi envoyer.</p>
<p><strong>Une bonne question fait deux choses à la fois : elle montre que tu t'intéresses vraiment, et elle donne envie de répondre.</strong> C'est tout ce qu'on demande à une conversation naissante, en fait. Les 100 questions qui suivent sont faites pour ça.</p>
<p>Elles sont classées par situation, du premier message au premier rendez-vous. On écrit « il » pour simplifier, mais tout fonctionne évidemment dans tous les sens.</p>`,
  quickSummary: [
    "100 questions classées par situation, du premier message au premier rendez-vous.",
    "La règle d'or : une question à la fois, et tu réponds aussi à celles que tu poses.",
    "Les questions drôles ouvrent la conversation, les questions perso la font durer.",
    "Le flirt marche mieux en sous-entendu qu'en frontal : les questions 66 à 80 sont calibrées pour ça.",
    "Ses réponses (et ses relances) en disent long sur son intérêt pour toi.",
  ],
  sections: [
    {
      id: 'par-message-pour-lancer',
      title: "Par message, pour lancer ou relancer (1-15)",
      content: `<p>Le format court, parfait pour un début de conversation ou pour réveiller un fil qui s'endort. Facile à envoyer, facile à répondre, et ça suffit largement pour commencer !</p>
<ol>
<li>Ta journée en trois emojis, sans tricher ?</li>
<li>Tu es plutôt vocal de 4 minutes ou pavé de texte ?</li>
<li>Qu'est-ce que tu écoutes en boucle en ce moment ?</li>
<li>Ta série du moment, celle que tu caches et celle que tu assumes ?</li>
<li>Si je te dis « le meilleur snack à 23 h », tu réponds quoi ?</li>
<li>Dernière photo de ton téléphone que tu peux m'envoyer sans honte ?</li>
<li>C'est quoi ton talent complètement inutile ?</li>
<li>Le dernier truc qui t'a fait rire tout seul dans la rue ?</li>
<li>Quelle appli tu supprimerais de ta vie si tu pouvais ?</li>
<li>C'est quoi la meilleure chose qui te soit arrivée cette semaine ?</li>
<li>Si on te payait pour faire une seule chose, ce serait quoi ?</li>
<li>Ton emoji le plus utilisé, on parie que je devine ?</li>
<li>La chanson qui te met de bonne humeur en dix secondes ?</li>
<li>Quel est ton dimanche parfait, heure par heure ?</li>
<li>Qu'est-ce que je devrais absolument savoir sur toi avant qu'on continue à parler ?</li>
</ol>
<aside class="blog-tip-box">
<p class="blog-tip-box-title">⚠️ Une question à la fois</p>
<p>Trois questions dans le même message, ça ressemble à un formulaire ! Envoie-en une, rebondis vraiment sur la réponse, et garde les autres pour plus tard. Une conversation de crush, c'est un ping-pong... pas un entretien d'embauche.</p>
</aside>`,
    },
    {
      id: 'pour-apprendre-a-le-connaitre',
      title: "Pour apprendre à le connaître (16-35)",
      content: `<p>Les fondamentaux. C'est là que tu découvres qui il est vraiment, au-delà du personnage des premiers échanges, et crois-moi, il y a souvent des surprises.</p>
<ol start="16">
<li>Qu'est-ce qui te passionne au point d'en perdre la notion du temps ?</li>
<li>Tu te décris comment en trois mots, et tes amis diraient lesquels ?</li>
<li>C'est quoi ton meilleur souvenir des cinq dernières années ?</li>
<li>Qu'est-ce que tu voulais faire comme métier à 8 ans ?</li>
<li>Tu es proche de ta famille, ou c'est compliqué ?</li>
<li>Le voyage qui t'a le plus marqué, c'était où et pourquoi ?</li>
<li>Qu'est-ce que tu fais quand tu as une journée rien qu'à toi ?</li>
<li>Plutôt grande bande d'amis ou deux personnes très proches ?</li>
<li>C'est quoi la chose la plus courageuse que tu aies faite ?</li>
<li>Qu'est-ce qui te met de mauvaise humeur à coup sûr ?</li>
<li>Tu as un rituel bizarre que personne ne connaît ?</li>
<li>C'est quoi ton endroit préféré dans ta ville ?</li>
<li>Le livre ou le film qui t'a changé quelque chose dans la tête ?</li>
<li>Tu crois en quoi, toi : le destin, le hasard, les efforts ?</li>
<li>Qu'est-ce que tu défendras toujours, même contre tout le monde ?</li>
<li>C'est quoi ta plus grande fierté, celle que tu ne racontes pas ?</li>
<li>Qu'est-ce qui te fait te sentir vivant ?</li>
<li>Tu préfères être admiré ou être compris ?</li>
<li>De quoi tes amis se moquent gentiment chez toi ?</li>
<li>Qu'est-ce que tu ferais de ta vie si tu étais sûr de réussir ?</li>
</ol>`,
    },
    {
      id: 'droles-pour-le-faire-rire',
      title: "Drôles, pour le faire rire (36-50)",
      content: `<p>L'humour, c'est le raccourci le plus court entre deux personnes qui se plaisent. Ces questions n'ont aucun enjeu, et c'est pour ça qu'elles marchent aussi bien !</p>
<ol start="36">
<li>C'est quoi ta théorie du complot préférée, celle que tu défends à moitié ?</li>
<li>Quel animal serait ton porte-parole officiel ?</li>
<li>Tu survivrais combien de temps dans un film d'horreur ?</li>
<li>Si ta vie était un film, ce serait quel genre, et qui jouerait ton rôle ?</li>
<li>Le pire cadeau qu'on t'ait jamais fait ?</li>
<li>Tu préfères te battre contre un canard géant ou cent canards minuscules ?</li>
<li>C'est quoi ton hot take alimentaire le plus scandaleux ?</li>
<li>La honte la plus mémorable de ta vie, celle qui te fait encore grimacer ?</li>
<li>Quel super-pouvoir complètement nul tu accepterais quand même ?</li>
<li>Ton pire message envoyé à la mauvaise personne ?</li>
<li>C'est quoi le mensonge le plus absurde que tu aies dit pour te sortir d'un truc ?</li>
<li>Tu donnerais quel prénom à un poisson rouge, sans réfléchir ?</li>
<li>Si les animaux pouvaient parler, lequel serait le plus insupportable ?</li>
<li>C'est quoi ta pire habitude que tu trouves quand même un peu géniale ?</li>
<li>On te donne un million, mais tu dois le dépenser en 24 heures sans rien investir : tu fais quoi ?</li>
</ol>`,
    },
    {
      id: 'un-peu-plus-perso',
      title: "Un peu plus perso (51-65)",
      content: `<p>À sortir quand la conversation est déjà bien lancée, et que tu sens que ça peut aller plus loin que les blagues. C'est souvent là, d'ailleurs, qu'un crush devient autre chose...</p>
<ol start="51">
<li>Qu'est-ce qui te rend vraiment heureux, pas sur le papier, en vrai ?</li>
<li>Tu parles facilement de ce que tu ressens, ou tu gardes tout ?</li>
<li>C'est quoi la leçon la plus dure que la vie t'ait apprise ?</li>
<li>De quoi tu as peur, sans que ce soit les araignées ?</li>
<li>Qu'est-ce que tu aimerais que les gens comprennent chez toi sans que tu aies à l'expliquer ?</li>
<li>Qu'est-ce qui t'a rendu comme tu es, en une histoire ?</li>
<li>Qu'est-ce que tu ne pardonnes pas facilement ?</li>
<li>Tu as déjà changé d'avis sur un truc important ? Raconte.</li>
<li>Qu'est-ce qui te fait te sentir en confiance avec quelqu'un ?</li>
<li>Qu'est-ce que tu fais quand ça ne va pas, ton vrai réflexe ?</li>
<li>Il y a un rêve que tu n'as jamais dit à voix haute ?</li>
<li>Tu te vois où dans cinq ans, sans la réponse toute faite ?</li>
<li>Qu'est-ce qu'on ne devine jamais en te voyant ?</li>
<li>Quelle personne a le plus compté dans ta vie jusqu'ici ?</li>
<li>Qu'est-ce que tu voudrais réparer, si tu pouvais ?</li>
</ol>`,
    },
    {
      id: 'pour-flirter-sans-trop-en-faire',
      title: "Pour flirter, sans trop en faire (66-80)",
      content: `<p>Le flirt qui fonctionne suit une règle toute simple : suggérer plutôt qu'annoncer. Ces questions envoient le signal, tout en laissant l'autre libre de le saisir... ou pas. C'est ce qui les rend efficaces sans jamais être lourdes.</p>
<ol start="66">
<li>Tu as remarqué quoi en premier chez moi, honnêtement ?</li>
<li>C'est quoi ton idée du rendez-vous parfait, sans budget illimité ?</li>
<li>Tu crois au coup de foudre, ou aux trucs qui se construisent ?</li>
<li>Qu'est-ce qui te fait craquer chez quelqu'un, la vraie réponse ?</li>
<li>Tu es plutôt premiers pas assumés ou signaux subtils ?</li>
<li>Qu'est-ce qui te rend curieux chez moi, là, maintenant ?</li>
<li>Si on passait une soirée ensemble, tu l'imaginerais comment ?</li>
<li>Tu retiens quoi des gens : les yeux, le rire, la voix ?</li>
<li>Tu préfères les débuts d'histoire ou les histoires installées ?</li>
<li>Il paraît que tout le monde a une technique de séduction. C'est quoi la tienne ?</li>
<li>Si je te proposais un truc improvisé demain, tu dirais oui ?</li>
<li>Qu'est-ce qui te ferait passer une mauvaise soirée, que je sache quoi éviter ?</li>
<li>Tu penses quoi des surnoms ? Question purement théorique.</li>
<li>Entre nous, tu réponds toujours aussi vite à tout le monde ?</li>
<li>Si cette conversation était un début de film, il s'appellerait comment ?</li>
</ol>
<aside class="blog-tip-box">
<p class="blog-tip-box-title">📌 Comment savoir si ça lui plaît</p>
<p>Regarde moins ses réponses que ses relances. Quelqu'un d'intéressé répond, puis te renvoie une question, rebondit, relance le lendemain. Quelqu'un de poli répond bien, et s'arrête là. La longueur des messages compte moins que tu ne crois : ce qui compte, c'est qui relance la machine.</p>
</aside>`,
    },
    {
      id: 'pour-voir-si-ca-pourrait-coller',
      title: "Pour voir si ça pourrait coller entre vous (81-92)",
      content: `<p>Sous leurs airs de conversation légère, ces douze questions vérifient l'essentiel : vos rythmes, vos attentes, vos visions. C'est le travail que font les <a href="/questions-couple/">questions de couple</a> une fois l'histoire lancée ; en version crush, on vérifie la même chose sans jamais poser LA question frontale.</p>
<ol start="81">
<li>C'est quoi ton week-end idéal : programme carré ou improvisation totale ?</li>
<li>Tu as besoin de beaucoup de temps pour toi, ou tu aimes la présence en continu ?</li>
<li>Tu es plutôt messages toute la journée ou retrouvailles le soir ?</li>
<li>Qu'est-ce qui te ferait fuir en début d'histoire ?</li>
<li>Tu dirais que tu es jaloux, honnête réponse ?</li>
<li>C'est quoi ta manière de montrer que quelqu'un compte pour toi ?</li>
<li>Plutôt grandes discussions ou complicité silencieuse ?</li>
<li>Qu'est-ce que tu attends d'une relation, dans l'idéal ?</li>
<li>Tu crois qu'on peut tout se dire, ou qu'il faut garder des jardins secrets ?</li>
<li>Ton niveau d'organisation sur une échelle de 1 à « j'ai un tableur » ?</li>
<li>Qu'est-ce qui te rassure chez quelqu'un que tu commences à apprécier ?</li>
<li>Si ça devait devenir sérieux entre nous, qu'est-ce qui devrait absolument rester comme maintenant ?</li>
</ol>`,
    },
    {
      id: 'au-premier-rendez-vous',
      title: "En vrai, au premier rendez-vous (93-100)",
      content: `<p>Huit questions à garder pour le face-à-face. Assez ouvertes pour faire parler, assez légères pour ne jamais plomber l'ambiance. Bref, ce qu'il faut un soir de premier rendez-vous.</p>
<ol start="93">
<li>Alors, c'est comment de me rencontrer en vrai par rapport aux messages ?</li>
<li>C'est quoi la chose la plus surprenante que tu aies apprise sur moi depuis qu'on parle ?</li>
<li>Qu'est-ce que tu fais après, si cette soirée se passe trop bien ?</li>
<li>Tu racontes quoi à tes amis sur moi, la vérité ?</li>
<li>Qu'est-ce qu'on fait la prochaine fois ? Question piège : il y a une prochaine fois.</li>
<li>Tu es plus stressé ou plus à l'aise que tu le pensais, là ?</li>
<li>Si tu devais me faire découvrir un endroit que tu adores, ce serait où ?</li>
<li>Bon. Sur dix, cette soirée ?</li>
</ol>
<p>Et si ce rendez-vous devient une histoire, tu verras le registre changer tout seul : les <a href="/blog/questions-a-poser-a-son-copain/">questions qu'on pose à son copain</a> n'ont plus grand-chose à voir avec celles qu'on posait à son crush, et c'est très bon signe.</p>`,
    },
    {
      id: 'et-s-il-ne-se-passe-rien',
      title: "Et si tu n'oses pas lui envoyer quoi que ce soit ?",
      content: `<p>Petit mot pour celle ou celui qui a lu jusqu'ici sans avoir envoyé le moindre message : c'est normal ! Un crush, par définition, fait perdre les moyens. Commence par les questions 1 à 15. Une seule. La plus simple, celle qui te ressemble. Le but n'est pas d'être brillant, c'est juste d'ouvrir une porte.</p>
<p>Et si vous parlez déjà tous les jours mais que tu n'arrives toujours pas à savoir si c'est réciproque, arrête de relire vos conversations pour la dixième fois. Franchement. Il y a des signaux bien plus fiables que ton interprétation de ses emojis à 1 heure du matin.</p>
<div><table><thead><tr><th>L'étape</th><th>Le bon registre</th><th>L'erreur classique</th></tr></thead><tbody>
<tr><td>Les premiers messages</td><td>Léger, court, une seule question à la fois</td><td>Le pavé qui attend une dissertation</td></tr>
<tr><td>Faire connaissance</td><td>Curieux, sur du concret</td><td>L'interrogatoire déguisé</td></tr>
<tr><td>Faire rire</td><td>Absurde, jamais aux dépens de l'autre</td><td>La blague qui teste les limites</td></tr>
<tr><td>Flirter</td><td>Suggéré, avec une porte de sortie</td><td>Le compliment qui met mal à l'aise</td></tr>
<tr><td>Le premier rendez-vous</td><td>Ouvert, et vraiment écouté</td><td>Réciter ses questions préparées</td></tr>
</tbody></table></div>
<p>Et si la question qui te bloque est plutôt de savoir ce que toi tu ressens, tu peux <a href="/test-amour-ou-crush/">faire le tri entre le crush et les vrais sentiments</a> en 35 questions.</p>
<div class="blog-cta">
<p class="blog-cta-titre">Il pense à toi, ou il répond juste poliment ?</p>
<p class="blog-cta-texte">Une série de questions passe en revue les vrais signaux, ceux qu'on ne remarque pas quand on a le nez dedans : ses relances, ses attentions, sa manière d'être quand tu es là. Résultat immédiat, et personne n'en saura rien.</p>
<a class="blog-cta-btn" href="/test-m-aime-t-il-elle-en-secret/">Découvrir ce qu'il ressent</a>
<p class="blog-cta-note">Gratuit &middot; Sans inscription &middot; 3 minutes</p>
</div>
<p>Dernier conseil, le plus important de tous : ces 100 questions sont des ouvertures, pas un script. Le jour où vous n'aurez plus besoin de liste pour vous parler pendant des heures... tu sauras que tu as bien joué !</p>
<a href="/blog/questions-a-poser-a-son-copain/" class="blog-read-also"><span class="blog-read-also-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg></span><span class="blog-read-also-content"><span class="blog-read-also-label">Lire aussi</span><span class="blog-read-also-title">100 questions à poser à son copain, des plus légères aux grandes conversations</span></span><svg class="blog-read-also-arrow" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14"/><path d="M12 5l7 7-7 7"/></svg></a>`,
    },
  ],
};

export default article;
