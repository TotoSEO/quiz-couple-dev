...
- `static-site/templates/partials/pub.ejs`, `static-site/js/pub.js` — Emplacements de la régie (The Moneytizer, site 142829) : 31 billboard sous le moteur, 2 pavé haut après la première section, 3 grand angle dans la colonne latérale (≥ 1440 px), 11 in text dans l'article de blog, 15 interstitiel et 6 footer dans base.ejs. Jamais deux fois le même format sur une page. Les scripts partent de pub.js au chargement complet de la page (`load`), puis à l'approche de l'écran pour chaque emplacement. Aucune publicité dans le premier écran : un emplacement qui tombe au-dessus de la ligne de flottaison au chargement, et le footer, attendent le premier défilement ; l'interstitiel ne part qu'au résultat (resultat-url.js), jamais au chargement. Un emplacement resté vide huit secondes se replie hors de l'écran (`.pub--vide`). Le footer est le seul format hors flux : son div (`.pub--6`, hauteur nulle) n'est qu'un point d'ancrage, le script de la régie ajoute en fin de body un conteneur fixe collé en bas (`#sas_iframe_fixed_26328`, 728x90 sur ordinateur, 320x50 ou 320x100 sur téléphone) ou un « slide-in » 300x250 au bord droit (`…-1`). pub.js le surveille : tout élément fixe de ce format qui dépasse 30 % de la hauteur de l'écran (standard Better Ads) reçoit `data-pub-trop-haut` et styles.css le masque ; tant qu'un bandeau est collé en bas, sa hauteur est dans `--pub-footer` sur `<html>` et le body en est dégagé en bas, pour qu'aucun bouton ne reste caché derrière ; la fermeture par la personne retire l'élément et la variable. Trois formats ont été retirés en septembre 2026 faute d'être vus (4 double skyrail, 28 megabanner bas, 19 pavé bas) ; les gouttières `--gouttiere-pub` (152 à 176 px à partir de 1024 px sur le hero, les avis, les tests liés, le pied de page et l'article de blog) creusées pour le skyrail sont encore là, à retirer dans un chantier à part. Le format « in text » se place tout seul dans l'article : les conteneurs du texte doivent tolérer un cadre inséré (`min-width: 0`, `max-width: 100%`).
...
un défilement, le footer compris ; le fichier de la plateforme de consentement part après
