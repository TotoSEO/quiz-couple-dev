...
- `static-site/templates/partials/pub.ejs`, `static-site/js/pub.js` — Emplacements de la régie (The Moneytizer, site 142829) : 31 billboard sous le moteur, 2 pavé haut après la première section, 3 grand angle dans la colonne latérale (≥ 1440 px), 11 in text dans l'article de blog, 15 interstitiel et 6 footer dans base.ejs. Jamais deux fois le même format sur une page. Les scripts partent de pub.js au chargement complet de la page (`load`), puis à l'approche de l'écran pour chaque emplacement. Aucune publicité dans le premier écran : un emplacement qui tombe au-dessus de la ligne de flottaison au chargement, et le footer, attendent le premier défilement. L'interstitiel (son div est sur toutes les pages publicitaires) ne part que dans deux cas : au résultat, sur le geste de la personne (resultat-url.js), ou à l'arrivée depuis une autre page du site, referrer de même origine (pub.js, `interstitielDeNavigation`), au plus un toutes les dix minutes, la date du dernier affichage étant dans localStorage sous `qc-interstitiel`, clé écrite dans les deux fichiers ; jamais à l'arrivée depuis l'extérieur, ni avec `?resultat` ou `?salon=` dans l'adresse. Un emplacement resté vide huit secondes se replie hors de l'écran (`.pub--vide`). Le footer est le seul format hors flux : son div (`.pub--6`, hauteur nulle) n'est qu'un point d'ancrage, le script de la régie ajoute en fin de body un conteneur fixe collé en bas (`#sas_iframe_fixed_26328`, 728x90 sur ordinateur, 320x50 ou 320x100 sur téléphone) ou un « slide-in » 300x250 au bord droit (`…-1`). pub.js le surveille : tout élément fixe de ce format qui dépasse 30 % de la hauteur de l'écran (standard Better Ads) reçoit `data-pub-trop-haut` et styles.css le masque, sauf un élément qui couvre presque tout l'écran (`estUnVoile`, 90 % en largeur et en hauteur) : le conteneur de l'interstitiel porte le même préfixe `sas_iframe_fixed_` (unité 26755, plein écran, création centrée avec sa croix), et le masquer laissait la page bloquée derrière l'`overflow:hidden` de la régie, sans rien de visible à fermer ; tant qu'un bandeau est collé en bas, sa hauteur est dans `--pub-footer` sur `<html>` et le body en est dégagé en bas, pour qu'aucun bouton ne reste caché derrière ; la fermeture par la personne retire l'élément et la variable. Trois formats ont été retirés en septembre 2026 faute d'être vus (4 double skyrail, 28 megabanner bas, 19 pavé bas), et avec le skyrail les gouttières `--gouttiere-pub` qui tenaient le hero, les avis, les tests liés, le pied de page et l'article de blog à 152 ou 176 px des bords sur bureau : ces blocs ont retrouvé leur largeur d'avant. Le format « in text » se place tout seul dans l'article : les conteneurs du texte doivent tolérer un cadre inséré (`min-width: 0`, `max-width: 100%`). Son div d'ancrage est en fin d'article, mais il est demandé au chargement complet (`DANS_LE_TEXTE` dans pub.js), pas à l'approche du div : demandé à l'approche, il ne partait qu'une fois l'article lu jusqu'au bout, un appel de script sur toute une période. Le script de la régie insère sa première annonce après le premier `<p>` de l'élément `article`, puis une tous les quatre paragraphes selon les enchères : le titre de l'encart « Résumé rapide » est donc un `div`, pas un `p`, pour que l'annonce tombe après le premier vrai paragraphe, sous la ligne de flottaison sur tous les écrans. Enfin, la régie recharge ses emplacements même hors écran (36 à 62 s selon l'unité, contre 19 à 27 s à l'écran) : pub.js pose vide la table `defaultRefreshTimeTableInvisible` de son objet `tmzrToolbox` avant son premier script (`coupeRafraichissementInvisible`), ce qui fait retomber chaque unité sur « jamais » hors écran sans toucher au rythme visible ; les taux de remplissage de 300 % du pavé haut venaient de là.
...
un défilement, le footer compris, et l'interstitiel seulement au résultat ou à l'arrivée depuis une autre page du site ; le fichier de la plateforme de consentement part après
